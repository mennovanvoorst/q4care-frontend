...
const Button = ({
  ...
  const classes = twMerge(
    `flex justify-center items-center font-medium transition duration-150 ease-in-out cursor:pointer disabled:cursor-not-allowed disabled:opacity-40 ${getFullWidth()} ${getSize()} ${getRounded()} ${getVariant()} ${
      className ?? ""
    }`
  );
  ...
  return (
    <button {...props} className={classes} disabled={disabled}>
      {showSpinner()}
      {showStartIcon()} {children} {showEndIcon()}
    </button>
  );
};
