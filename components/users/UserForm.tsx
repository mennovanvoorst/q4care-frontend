...
const roleOptions = [
  ...
  {
    value: (USER_ROLES.paid + USER_ROLES.student).toString(),
    label: "Student",
  },
  {
    value: (
...
const UserForm = ({ defaultUser }: Props) => {
  ...
  const [skills, setSkills] = useState(
    defaultUser
      ? defaultUser.skills.map((skill: any) => ({
          id: skill.id,
          name: skill.name,
        }))
      : []
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      let response;
      if (defaultUser) {
        response = await UserAPI.updateById(
          defaultUser.id,
          data.firstName,
          data.lastName,
          data.email,
          parseInt(data.flags, 10)
        );
      } else {
      ...
      setAlert({
        type: "success",
        message: "De gebruiker is toegevoegd/aangepast!",
      });
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      setLoading(true);
      let response = await UserAPI.destroy(defaultUser.id);
  ...
  const handleRemoveSkill = (skill: any) => {
    setSkills((prev: any[]) =>
      prev.filter((oldSkill) => oldSkill.id !== skill.id)
    );
    ...
    <>
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <TextInput
          ...
          className="bg-white text-slate-800"
          placeholder="email"
          register={register("email", { required: "Email is verplicht" })}
        ...
        <Select
          ...
          className="bg-white text-slate-800"
          placeholder="Flags"
          register={register("flags", { required: "Rol is verplicht" })}
        ...
        <Datalist
          ...
          options={skillsList.map((skill: any) => ({
            value: skill.id,
            label: skill.name,
          }))}
        />
      ...
      {defaultUser && (
        <Modal
          ...
          content={`Weet je zeker dat je ${defaultUser.firstName} wilt verwijderen?`}
