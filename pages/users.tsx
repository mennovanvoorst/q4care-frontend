...
const Users: NextPage = ({ users }: any) => {
  const renderList = () => {
    ...
    return users.map((user: any) => <UserCard id={user.id} name={`${user.firstName} ${user.lastName}`} email={user.email} hasPaid={checkRole(user.flags, USER_ROLES.paid) || checkRole(user.flags, USER_ROLES.student)} isTeacher={checkRole(user.flags, USER_ROLES.teacher)} isAdministrator={checkRole(user.flags, USER_ROLES.admin)} />)
  }
